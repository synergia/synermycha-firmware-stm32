...
template <typename R, typename... Args> class FunctionPtr
{
  public:
    ...
    template <typename T, R (T::*ptr)(Args...)> static FunctionPtr bind(Observer* obj)
    {
...
{
  public:
    template <typename T, R (T::*ptr)(Args...)> void connect(Observer& obj)
    {
        funs[actualNrOfElems++] = (FunctionPtr<R, Args...>::template bind<T, ptr>(&obj));
    }

    template <typename... ArgsRefs> void emit(ArgsRefs&&... args)
    {
        for (auto& f : funs)
        {
