    ...
    SSD1306_Command(
        ...
        (0x08 &
            (Rotate << 3))); // Set COM Output Scan Direction
                             ...
                             // COM[N-1];Where N is the Multiplex ratio. 0xC8	(0xC8) => remapped
                             // mode. Scan from COM[N-1] to COM0;;Where N is the Multiplex ratio.
}
...
void SSD1306_ScrollingHorVer(uint8_t Direction, uint8_t StartPage, uint8_t EndPage,
    uint8_t HorizontalSpeed, uint8_t VerticalOffset)
{
    if (VerticalOffset > 127)
        VerticalOffset = 127;
...
void SSD1306_StartScrollLeftUp(
    uint8_t StartPage, uint8_t EndPage, uint8_t HorizontalSpeed, uint8_t VerticalOffset)
{
    SSD1306_ScrollingHorVer(SSD1306_VERTICAL_AND_LEFT_HORIZONTAL_SCROLL, StartPage, EndPage,
        HorizontalSpeed, VerticalOffset);
}

void SSD1306_StartScrollRightUp(
    uint8_t StartPage, uint8_t EndPage, uint8_t HorizontalSpeed, uint8_t VerticalOffset)
{
    SSD1306_ScrollingHorVer(SSD1306_VERTICAL_AND_RIGHT_HORIZONTAL_SCROLL, StartPage, EndPage,
        HorizontalSpeed, VerticalOffset);
}
...
void SSD1306_Init(void)
{
    ...
    SSD1306_Command(
        0x12); // Alternative COM Pin---See IC Spec page 34
...
void SSD1306_Display(void)
{
    ...
    if (ssd1306_spi->hdmatx->State == HAL_DMA_STATE_READY)
    {
        HAL_SPI_Transmit_DMA(
            ssd1306_spi, (uint8_t*)&buffer, (SSD1306_LCDHEIGHT * SSD1306_LCDWIDTH / 8));
    }
#else
    HAL_GPIO_WritePin(SSD1306_DC_GPIO_Port, SSD1306_DC_Pin, GPIO_PIN_SET);
#ifndef SPI_CS_HARDWARE_CONTROL
...
#endif
    HAL_SPI_Transmit(
        ssd1306_spi, (uint8_t*)&buffer, (SSD1306_LCDHEIGHT * SSD1306_LCDWIDTH / 8), 100);
#ifndef SPI_CS_HARDWARE_CONTROL
    HAL_GPIO_WritePin(SSD1306_CS_GPIO_Port, SSD1306_CS_Pin, GPIO_PIN_SET);
#endif
#endif
...
void SSD1306_Bitmap(uint8_t* bitmap)
{
    ...
    if (ssd1306_i2c->hdmatx->State == HAL_DMA_STATE_READY)
    {
        HAL_I2C_Mem_Write_DMA(ssd1306_i2c, SSD1306_I2C_ADDRESS, 0x40, 1, bitmap,
            (SSD1306_LCDHEIGHT * SSD1306_LCDWIDTH / 8));
    }
#else
    HAL_I2C_Mem_Write(ssd1306_i2c, SSD1306_I2C_ADDRESS, 0x40, 1, bitmap,
        (SSD1306_LCDHEIGHT * SSD1306_LCDWIDTH / 8), 100);
#endif
#endif
#ifdef SSD1306_SPI_CONTROL
    ...
    !defined(SPI_CS_HARDWARE_CONTROL)
