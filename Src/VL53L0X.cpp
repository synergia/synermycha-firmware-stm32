...
bool init(VL53L0X* lidar, bool io_2v8)
{
    ...
    // snprintf(msg,sizeof(msg),"Stop variable is: 0x%X , should be
    // 0x%X\r\n",lidar->stop_variable,readReg(lidar,0x91)); HAL_UART_Transmit(&huart2, (uint8_t*)msg,
    // strlen(msg), 0xFFFF);
    ...
    if (!getSpadInfo(lidar, &spad_count, &spad_type_is_aperture))
    {
        return false;
    }
    ...
    // snprintf(msg,sizeof(msg),"Spads: 0x%X, 0x%X, 0x%X, 0x%X, 0x%X,
    ...
    if (!performSingleRefCalibration(lidar, 0x40))
    {
        return false;
    }
    ...
    if (!performSingleRefCalibration(lidar, 0x00))
    {
        return false;
    }
